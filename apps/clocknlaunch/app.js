...
var apps = s.list(/\.info$/).map(app=>{
  ...
  return a && {
    ...
    app=>app && (app.type=="app" || (app.type=="clock" && dtsettings.showClocks) || (app.type=="launch" && dtsettings.showLaunchers) || !app.type));
...
function drawPage(p){
    ...
    for (var i=0;i<4;i++) {
        ...
        draw_icon(p,i,selected==i && !dtsettings.direct);
    }
    g.flip();
}
...
for (const key in saved_settings) {
  bwsettings[key] = saved_settings[key]
}
...
function getMenuEntry(){
  ...
  bwsettings.menuPosX = bwsettings.menuPosX % menu.length;
  bwsettings.menuPosY = bwsettings.menuPosY % menu[bwsettings.menuPosX].length;
  var menuEntry = menu[bwsettings.menuPosX][bwsettings.menuPosY]();
  ...
  if(typeof menuEntry[0] !== 'function'){
    ...
    menuEntry[0] = function(callbackFun){
      callbackFun(String(value), bwsettings.menuPosX, bwsettings.menuPosY);
    }
  }
  return menuEntry;
}
...
function isFullscreen(){
  var s = bwsettings.screen.toLowerCase();
  if(s == "dynamic"){
...
function measureAltitude(callbackFun){
  var oldX = bwsettings.menuPosX;
  var oldY = bwsettings.menuPosY;
  try{
...
function drawTime(){
  ...
  var colon = bwsettings.hideColon ? "" : ":";
  ...
  var printImgLeft = bwsettings.menuPosY != 0;
  ...
  menuTextFun((menuText, oldX, oldY) => {
    ...
    if(bwsettings.menuPosX != oldX || bwsettings.menuPosY != oldY){
      return;
    }
...
function drawLock(){
  if(bwsettings.showLock && Bangle.isLocked()){
    g.setColor(g.theme.fg);
...
Bangle.on('lock', function(isLocked) {
  ...
  if(!isLocked && bwsettings.screen.toLowerCase() == "dynamic"){
...
E.on("kill", function(){
  try{
    storage.write(SETTINGS_FILE, bwsettings);
  } catch(ex){
